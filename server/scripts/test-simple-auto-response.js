...
async function testAutoResponseDirectly() {
  try {
    ...
    chatsResponse.data.forEach((chat, index) => {
      ...
      console.log(`  ${index + 1}. ${chat.name || 'Unnamed'} (${participantNames}) - ID: ${chat._id}`);
    });
    ...
    let originalMessage = null;    socket.on('connect', () => {
      console.log('✅ Alice connected to WebSocket');

      // Wait a moment for authentication to complete before joining chat
      setTimeout(() => {
        // Join the chat
        ...
        if (response?.success) {
          ...
          setTimeout(() => {
            ...
            const testMessage = 'Hello! This should trigger an auto-response from the bot.';
            socket.emit('sendMessage', {
              ...
              if (messageResponse?.success) {
                ...
                originalMessage = messageResponse.message;
                console.log('⏳ Waiting for auto-response (should arrive in ~2 seconds)...');
              } else {
    ...
    setTimeout(() => {
      ...
      console.log(`   Bot message received: ${botMessageReceived ? '✅ YES' : '❌ NO'}`);
    ...
    }, 8000); // Wait 8 seconds for auto-response
      }, 500); // Wait 500ms for authentication
    });
...
async function checkChatMessages(chatId, token) {
  try {
    ...
    const messagesResponse = await axios.get(`${BASE_URL}/messages/chat/${chatId}`, {
    ...
    recentMessages.forEach((msg, index) => {
      ...
      console.log(`  ${index + 1}. ${msg.sender.name}${isBot ? ' 🤖' : ''}: "${msg.content}"`);
      console.log(`     Time: ${new Date(msg.createdAt).toLocaleTimeString()}`);
    });

    const botMessages = messagesResponse.data.filter(msg => msg.sender.email === 'chatbot@angular-chat.com');
